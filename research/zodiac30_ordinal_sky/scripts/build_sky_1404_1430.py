...
def angular_sep(a,b,t):
    xa,xb=calc(a,t),calc(b,t)
    la,ba,lb,bb=map(math.radians,[xa[0],xa[1],xb[0],xb[1]])
    c=math.sin(ba)*math.sin(bb)+math.cos(ba)*math.cos(bb)*math.cos(la-lb)
...
def ecl_type(flags,lunar=False):
    ...
    return 'unknown'
def refine_longitude_conjunction(a,b,t0):
        ...
        for b in VISIBLE[i+1:]:
            ...
            for k in range(1,len(grid)-1):
                if ds[k]<=ds[k-1] and ds[k]<ds[k+1] and ds[k]<1.0:
                    t,dlon=refine_longitude_conjunction(a,b,float(grid[k]))
                    if not cand or t-cand[-1][0]>3:cand.append((t,dlon))
                    elif dlon<cand[-1][1]:cand[-1]=(t,dlon)
            for t,dlon in cand:
                l=(lon(a,t)+((lon(b,t)-lon(a,t)+180)%360-180)/2)%360;elong=adiff(l,lon('Sun',t))
                out.append({'body_a':a,'body_b':b,'date':date_string(t),'jd_ut':t,'longitude_difference_deg':dlon,'angular_separation_deg':angular_sep(a,b,t),'ecliptic_longitude_deg':l,'solar_elongation_deg':elong,'event_class':'great_conjunction' if {a,b}=={'Jupiter','Saturn'} else 'longitude_conjunction'})
...
def main(outdir):
    ...
    write_csv(outdir/'planetary_longitude_conjunctions.csv',con);write_csv(outdir/'solar_eclipses_reference_locations.csv',sol);write_csv(outdir/'lunar_eclipses_reference_locations.csv',lun)
    old=outdir/'planetary_conjunctions_under_1deg.csv'
    ...
    for day in np.arange(jd(1425,1,1),jd(1426,1,1),1):
        y,m,d,h=rev(float(day));daily.append({'date':f'{y:04d}-{m:02d}-{d:02d}','jd_ut':float(day),'Sun':lon('Sun',day),'Moon':lon('Moon',day),'Jupiter':lon('Jupiter',day),'Saturn':lon('Saturn',day),'Jupiter_Saturn_longitude_difference_deg':adiff(lon('Jupiter',day),lon('Saturn',day)),'Jupiter_Saturn_angular_separation_deg':angular_sep('Jupiter','Saturn',day)})
    ...
    fig,ax=plt.subplots(figsize=(12,5));ax.plot(x,[r['Jupiter'] for r in daily],label='Jupiter');ax.plot(x,[r['Saturn'] for r in daily],label='Saturn');ax.set(xlabel='Day of 1425',ylabel='Ecliptic longitude (deg)',title='Triple Jupiter–Saturn longitude conjunction in Scorpio, 1425');ax.legend();fig.tight_layout();fig.savefig(outdir/'jupiter_saturn_triple_1425.svg');plt.close(fig)
    major_solar=[];bydate={}
    ...
    summary={'method':{'calendar':'Julian','ephemeris':'Swiss Ephemeris / Moshier','range':'1404-01-01 through 1430-12-31','reference_locations':LOCATIONS,'conjunction_definition':'equality of geocentric tropical ecliptic longitude; full angular separation reported separately'},'great_conjunctions':great,'major_solar_eclipses_reference_belt':major_solar,'total_lunar_eclipses_visible_reference_belt':total_lunar,'precession_across_26_years_deg':26*50.29/3600}
    (outdir/'ASTRONOMY_RESULT.json').write_text(json.dumps(summary,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
    lines=['# Astronomy 1404–1430: computed findings','','Calculations use the Julian calendar and Swiss Ephemeris/Moshier. Reference locations are sampling points, not a provenance claim. A conjunction here means equality of geocentric tropical ecliptic longitude; full spherical angular separation is reported separately.','','## Jupiter–Saturn','']
    for r in great:lines.append(f"- {r['date']}: longitude difference {r['longitude_difference_deg']:.5f}°, angular separation {r['angular_separation_deg']:.3f}°, longitude {r['ecliptic_longitude_deg']:.2f}°, solar elongation {r['solar_elongation_deg']:.1f}°.")
    lines+=['','The 1425 event is a triple longitude conjunction in tropical Scorpio and is the strongest historically specific candidate in the manuscript date window. The planets were about 1.18–1.26° apart on the sphere. The 1405 conjunction was only about 18° from the Sun and was much less observable.','','## Major solar eclipses in the European reference belt','']
